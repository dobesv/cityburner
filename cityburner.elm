...
foreign import jsevent "provideRobotAnimation" (castElementToJSElement (spacer 0 0)) jsRobotAnimation : Signal JSElement

robotAnimation = lift (castJSElementToElement 262 373) jsRobotAnimation

-- Return Just head for a non-empty list, or Nothing for an empty list
headMaybe : [x] -> Maybe x
headMaybe lst =
    case lst of
        [] -> Nothing
...
cropImage : [Char] -> Int -> Int -> Int -> Int -> Int -> Int -> Element
...
crop : Element -> Int -> Int -> Int -> Int -> Element
...
bldgSheet : {width:Int,height:Int,src:[Char]}
bldgSheet = {width=1800,height=400,src="images/CityBurner_Buildings_V2.png"}
type Bldg = {width:Int,height:Int,image:Element,hitMask:Form}
bldg : (Int,Int,Int,Int,[(Int,Int)]) -> Bldg
...
type Fire = {worldX:Int,worldY:Int,since:Int}

type GameState = {oldFires:[Fire]}
type JSElts = {robot:Element}
type GameInput = {t:Int, mouseDown:Bool, mousePosition:(Int,Int), jselts:JSElts}
updateGame : GameInput -> GameState -> (Element,GameState)
...
mkjselts : Element -> JSElts
mkjselts r = {robot=r}
jselts : Signal JSElts
jselts = lift mkjselts robotAnimation
mkinput : Int -> Bool -> (Int,Int) -> JSElts -> GameInput
